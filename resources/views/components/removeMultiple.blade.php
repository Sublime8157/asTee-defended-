...
<th>
    ...
    {{-- confirmation for move  --}}
    <dialog id="confirmationMove" class="modal dialog w-80 p-2 bg-white rounded"> 
        <div class="flex flex-col">
            <div class="p-4">
                <p class="text-xs text-center font-normal text-red-500">You are about to move selected products, that might affect multiple tables, are you sure you want to update these?</p>
            </div>
            {{-- update Form --}}
            ...
            <div class="flex justify-between items-center flex-row ">
                <div class="w-40   hover:opacity-70 border-r">
                    <button class="py-1 font-normal text-sm" onclick="document.getElementById('confirmedMove').submit()">Confirm</button>
                </div>
                <div class="cursor-pointer hover:opacity-70 w-full bg-orange-400 text-white font-normal text-sm ">
                    <button class="py-1 " onclick="document.getElementById('confirmationMove').close()" >Cancel</button>
                </div>
            </div>
        </div>
    </dialog>
